README

Name - HTML status - AJAX status

—————————————————————————————————
Pages in seller's website:

* LoginPageSeller - done - done
* RegistrationSeller - done - done
* PayMemberFee - done - done

* HomePageSeller - done - done
* BusinessPage - done - done
EditBusinessProfile - done - Florence
ManageAdvertisementSeller - Artem
* StatisticsCenter - done - done
TransferPointsSeller - done - 
ExchangePointsSeller - done - 
Intelligent Query - done - 
TradingChartPointsSeller - done - 

Pages linked to seller's system:

LoginSellerSystem - done - 
RegistrationInCompany - done - 
CompanyPresentation - done - 
CompanyPresentation 2 - done - 
* ListOfSellersPage - done - done
* ListOfSellersPage2 - done - done
—————————————————————————————————
Pages in user’s website:
* MainPage - done - done
OffersStreaming - done - Helen
OffersStreaming2 - done - Helen
LoginPage - done - 
RegistrationPage - 

* Homepage - done - to be tested
MyAccount - done - Helen
EditProfile - done - Quentin
MyPointsPage - done - to be tested 
* ExchangeCenter - done - done
Makeoffer - done - 
RecentActivity - done - 
findoffer - done - 
ExchangesFound - done - to be tested
ExchangeRatePage - 
UserProfilePage - done - 
ExchangeStatus - done -
SelectUserPage - done - 
HistoryPage - done - to be tested
Notification - to be tested